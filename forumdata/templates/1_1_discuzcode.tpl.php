...
EOF;
 if(strstr(strtolower($attach['filename']), 'utf') !== FALSE) { 
$return .= <<<EOF

      			<script language='JavaScript'>new GameController().load('/bbs/attachment.php?aid={$aidencode}&encoding=utf-8');</script>
    		
EOF;
 } else { 
$return .= <<<EOF

      			<script language='JavaScript'>new GameController().load('/bbs/attachment.php?aid={$aidencode}');</script>
    		
EOF;
 } } elseif(!isset($attach['unpayed'])) { if($attach['attachimg']) { if(!IS_ROBOT) { 
$return .= <<<EOF
