<? if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('stats_team');
0
|| checktplrefresh('/data/apps/bbs/releases/7.2_20100331/././templates/default/stats_team.htm', '/data/apps/bbs/releases/7.2_20100331/././templates/default/header.htm', 1310957590, '1', './templates/default')
|| checktplrefresh('/data/apps/bbs/releases/7.2_20100331/././templates/default/stats_team.htm', '/data/apps/bbs/releases/7.2_20100331/././templates/default/stats_navbar.htm', 1310957590, '1', './templates/default')
|| checktplrefresh('/data/apps/bbs/releases/7.2_20100331/././templates/default/stats_team.htm', '/data/apps/bbs/releases/7.2_20100331/././templates/default/footer.htm', 1310957590, '1', './templates/default')
|| checktplrefresh('/data/apps/bbs/releases/7.2_20100331/././templates/default/stats_team.htm', '/data/apps/bbs/releases/7.2_20100331/././templates/default/jsmenu.htm', 1310957590, '1', './templates/default')
;?>
...
<h3 onclick="toggle_collapse('list_stats_team_admins', 1, 1);">管理员和超级版主</h3>
<div class="c_header_action">
<p class="c_header_ctrlbtn" onclick="toggle_collapse('list_stats_team_admins', 1, 1);">[ 展开 ]</p>
</div>
</div>
<table id="list_stats_team_admins" summary="管理员和超级版主" cellspacing="0" cellpadding="0">
...
<td>积分</td>
<td>帖子</td>
<td width="50">最近 30 天发帖</td>
<? if($modworkstatus) { ?><td width="30">本月管理</td><? } if($oltimespan) { ?>
<td width="50">总计在线(小时)</td>
<td width="50">本月在线(小时)</td>
<? } ?>
</tr>
</thead><? if(is_array($team['admins'])) { foreach($team['admins'] as $uid) { ?><tr class="<? echo swapclass('colplural'); ?>">
...
<? } if(is_array($team['categories'])) { foreach($team['categories'] as $category) { ?><div class="datalist">
...
<div class="c_header_action">
<p class="c_header_ctrlbtn" onclick="toggle_collapse('list_category_<?=$category['fid']?>', 1, 1);">[ 展开 ]</p>
</div>
</div>
...
</thead><? unset($swapc); if(is_array($team['forums'][$category['fid']])) { foreach($team['forums'][$category['fid']] as $fid => $forum) { if(is_array($team['moderators'][$fid])) { foreach($team['moderators'][$fid] as $key => $uid) { ?><tr class="<? echo swapclass('colplural'); ?>">
<? if($key == 0) { ?><td rowspan="<?=$forum['moderators']?>" class="nobg"><? if($forum['type'] == 'group') { ?><a href="<?=$indexname?>?gid=<?=$fid?>"><? } else { ?><a href="forumdisplay.php?fid=<?=$fid?>"><? } ?><?=$forum['name']?></a></td><? } ?>
