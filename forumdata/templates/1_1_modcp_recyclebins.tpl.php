...
<option value=""></option><? if(is_array($modforums['list'])) { foreach($modforums['list'] as $id => $name) { if($modforums['recyclebins'][$id]) { ?>
<option value="<?=$id?>" <? if($id == $fid) { ?>selected<? } ?>><?=$name?></option>
<? } } } ?></select>
</td>
<td width="15%">帖子类型</td>
<td width="35%">
...
<? if($fid) { ?>
<div class="c_header"><h2>当前版块: <a href="forumdisplay.php?fid=<?=$fid?>" target="_blank" class="lightlink"><?=$forum['name']?></a></h2></div>
...
<button onclick="modthreads('restore')">恢复主题</button>
</th>
</tr>
</tbody>
<? } ?>
...
<? if($multipage) { ?><?=$multipage?> <? } if(!$total) { ?><p class="nodata">没有找到相关主题</p><? } ?>
</div>
